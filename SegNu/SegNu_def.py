...
def CNN_inference(dim_patch, num_regions):
    ...
    validation_generator = predict_data.flow_from_directory(samples_data_dir,
                                                            ...
                                                            shuffle = False)
    predictions = []
    for i in range(num_regions//batch_size):
        pred_model = model.predict(next(validation_generator)[0:batch_size])
        predictions.append(pred_model.argmax(axis=1).tolist())
    
    if num_regions%batch_size!=0:
        pred_model = model.predict(next(validation_generator)[0:num_regions%batch_size])
        predictions.append(pred_model.argmax(axis=1).tolist())
...
def set_arg(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            if arg not in ["clustering", "watershed", "only_limit_threshold","watershed_no_cnn"]:
                print("Error")
...
def AC (binary_image, original_image):
    ...
    lcc = label(bin_image)
    area = [i.area for i in regionprops(lcc,image)]
    region = regionprops(lcc,image)[area.index(max(area))]
    AC = AgglomerativeClustering(n_clusters = 2, linkage = "average").fit(region.coords)
    cast = np.zeros((image.shape[0],image.shape[1]))
...
def run_watershed_seg(threshold_steps,
    ...
    if threshold_steps == [] or cluster_regions == []:
...
def run_clustering_seg(unicell_regions,
    ...
    if cluster_regions == []:
        ...
        return (run_clustering_seg(unicell_regions, cluster_regions, label_image, raw_image, patch))
    ...
    for path_image in images_list:
        ...
        if segmentation=="only_limit_threshold" or segmentation=="watershed_no_cnn":
            ...
            if segmentation=="watershed":
                ...
                segmented_image, unicell_regions = run_clustering_seg(unicell_regions,cluster_regions,label_image, image, patch)
